...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { name, description, price, productCategory, thumbnails, ownerId } = req.body;

    const owner = await UserModel.findById(ownerId);
    if (!owner) {
      return res.status(404).json({ message: 'Pemilik produk tidak ditemukan.' });
    }
...
router.get('/', async (req: Request, res: Response) => {
  try {
    const products = await ProductModel.find().populate('owner', 'name'); // Menggunakan populate untuk mengambil data pemilik produk (field 'owner') dan hanya memuat properti 'name'
    res.json(products);
  } catch (error) {
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const product = await ProductModel.findById(req.params.id).populate('owner', 'name'); // Menggunakan populate untuk mengambil data pemilik produk (field 'owner') dan hanya memuat properti 'name'
    if (!product) {
